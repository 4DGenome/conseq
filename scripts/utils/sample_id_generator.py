...
technical_fields = ['HIC', 'SEQUENCING_PRE_LIBRARY', 'SEQUENCING_LIBRARY', 'SEQUENCING_CORE', 'SEQUENCING_RUN']
...
def assign_sample_id(df):
	...
	# select samples without SAMPLE_ID
 	if mode == 'all_samples':
	 	cond1 = df['PROJECT'] == '4DGENOME'
 		samples = df[cond1].reset_index(drop = True)
 	elif mode == 'missing_sample_ids':
	 	df['SAMPLE_ID'].fillna(0, inplace=True)
	 	cond1 = df['PROJECT'] == '4DGENOME'
 		cond2 = df['SAMPLE_ID'] == 0
 		samples = df[cond1 & cond2].reset_index(drop = True)	 
